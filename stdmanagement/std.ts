...
const randomNo: number = Math.floor(10000 + Math.random() * 80000); //STUDENTID
// console.log(randomNo)
let mymoneyBalance: number = 0;
...
const coursetuitionfees: { [key: string]: number } = {
  GraphicDesign: 5000,
...
if (Tuitionfees == paymentAmount) {
  ...
  if (Answer.Select === "View Current Status") {
    ...
    console.log(chalk.blueBright.italic(`StudentName:${ans.Students}`));
    console.log(chalk.blueBright.italic(`StudentID: ${randomNo}`));
    console.log(chalk.blueBright.italic(`Institute/Academy Name: ${ans.Institute}`));
